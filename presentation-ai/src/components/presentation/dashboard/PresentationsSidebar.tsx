...
import { deletePresentationsAPI } from "@/lib/fastapi-client";
import { fetchPresentations } from "@/app/_actions/presentation/fetchPresentations";

// Type based on FastAPI PresentationResponse
...
export function PresentationsSidebar() {
  ...
  const { mutate: deleteSelectedPresentations } = useMutation({
    mutationFn: async () => {
      const result = await deletePresentationsAPI(selectedPresentations);
